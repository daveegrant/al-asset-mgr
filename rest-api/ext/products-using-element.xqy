...
module namespace ext = "http://marklogic.com/rest-api/resource/products-using-element";
...
(:
 :)
declare
%roxy:params("source=xs:string,sourceTrackingNumber=xs:string,bookCode=xs:string,costType=xs:string")
function ext:get(
  $context as map:map,
  $params  as map:map
) as document-node()*
...
) as document-node()*
{
  ...
  let $src-track-num := map:get($params, "sourceTrackingNumber")
  ...
  let $element-queries := (
    ...
    cts:element-range-query(xs:QName("Source_Tracking_Number"), "=", $src-track-num),
    if ($book-code) then
  ...
  let $_ :=
    ...
    return
      ...
      let $_ := map:put($el, "author", $element/source/element/Author_Last_Name/data())
  ...
  let $book-codes :=
    if ($book-code) then
      ...
      fn:distinct-values(
        for $element in $elements
        order by $element/commonBookCode
  ...
  let $_ := map:put($response, "sourceTrackingNumber", $src-track-num)
  let $_ :=
    if ($book-code) then
