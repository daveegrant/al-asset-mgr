...
<row r="{$row + 1}" spans="1:{fn:count($cols)}">{
  for $c at $pos in $cols
  let $value := map:get($cellmap,$c)
  let $a1-ref := map:get($INDEX-ALPHA-MAP,fn:string($pos)) || $row + 1
  return
    if($value) then 
...
) as binary()?
{
	...
	let $element-names :=
	  if($headers) then 
		for $column at $pos in $headers/child::*
		let $name := fn:local-name($column)
		return 
	        $name
      else 
	    for $column at $pos in $body-rows[1]/child::*
	    let $name := fn:local-name($column)
		return 
			$name
	...
	let $sheet1 := excel:worksheet(($headers,$body-rows), $col-widths, $tbl-count)
