...
module namespace trans = "http://marklogic.com/rest-api/transform/ingest-element-xlsx";
...
) as map:map*
{
  ...
  let $collections := ("data", "xlsx-element")
  let $the-doc := map:get($content, "value")

  let $_ := xdmp:log(xdmp:binary-size($the-doc))
  let $_ := xdmp:log("Params " || $params)

  let $res := cs-lib:process-excel-data(trans:get-uri-prefix($params), $the-doc, 0, $permissions, $collections, fn:false())
  let $_ := xdmp:log($res)
