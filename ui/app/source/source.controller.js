...
(function () {
  'use strict';
  angular.module('app.source')
  .controller('SourceCtrl', SourceCtrl);

  SourceCtrl.$inject = ['doc', '$stateParams', 'MLRest', '$filter'];
  function SourceCtrl(doc, $stateParams, mlRest, $filter) {
    var ctrl = this;
    ...
    ctrl.updateChartData = function(data) {
      ...
      for (var i=0; i < data.matchingProducts.length; i++) {
        var url = 'source?id=' + $filter('escape')(id)
      ...
      ctrl.bar2ChartConfig.series[0].data = bookData;
      ctrl.pieChartConfig.series[0].data = bookData;
